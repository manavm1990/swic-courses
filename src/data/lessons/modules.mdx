...
You can export multiple things from a file by name. In modern JavaScript, you'll often see **arrow functions** exported this way:
...
/**
 * Example: filter a list of Pokémon by type.
 *
 * - Arrow function
 * - Exported as a named ESM export
 * - Uses camelCase and starts with a verb because functions perform an action
 */
...
/**
 * Map a list of Pokémon objects to an array of just their names.
 */
...
In another file, you can import these helpers:

```js
import { add, subtract, filterByType, getPokemonNames } from "./math.js";
```
