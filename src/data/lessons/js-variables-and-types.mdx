...
- When you call a method on a primitive (like `"hello".toUpperCase()`), JavaScript temporarily wraps it in a `String`/`Number`/`Boolean` object so the method call can work, then returns a **new primitive** and discards the wrapper.
