...
You've written JavaScript before. You know `const x = 5` creates a variable. But after months away, it's the subtle details that slip - the difference between `null` and `undefined`, why `[] == false` is true, or why modifying an object affects all variables pointing to it.
...
Open your browser's Developer Console now (F12 → Console tab). Everything we do today happens there.
