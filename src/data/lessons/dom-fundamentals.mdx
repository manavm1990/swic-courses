...
When using `querySelector("#some-id")`, you **include** the `#` character yourself. With `getElementById("some-id")`, you just pass the bare id name (`"some-id"`) without the `#`.
...
Instead of directly changing styles with `element.style`, Tailwind encourages you to add or remove **utility classes** (like `bg-red-500`, `hidden`, `text-xl`) on the element. The `classList` API makes this easy.
...
// classList is another PROPERTY on the element object
// It has METHODS like add, remove, toggle, and contains
...
// Add to a specific container on the page
const container = document.querySelector("#container");
container.appendChild(newDiv);

// Or, append directly to the document body
// (common for small "Hello" examples or quick experiments)
...
Common issue: `document.querySelector()` returns `null`. This happens when your **CSS selector doesn't match any node on the page**—for example, the class/id changed in the HTML, the element hasn't been rendered yet, or there's a typo in your selector.
...
For example, when you see `heading.textContent` or `button.addEventListener("click", handleClick)`, **`textContent` is a property** on the DOM element object, and **`addEventListener` is a method** (a function stored as a property that you can call).
