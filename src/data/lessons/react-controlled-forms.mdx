...
<Accordion question="What's happening here?">
  <ol>
    <li>User types "H" → `onChange` fires → `setMessage("H")` runs</li>
    <li>
      React re-renders → `value={message}` shows "H" → Character count shows `1`
    </li>
    <li>User types "e" → `onChange` fires → `setMessage("He")` runs</li>
    <li>
      React re-renders → `value={message}` shows "He" → Character count shows
      `2`
    </li>
    <li>And so on...</li>
  </ol>
  <strong>Key concept:</strong> The data flows in a circle: User types → State
  updates → UI updates → User sees new UI → User types again...
</Accordion>
...
**Important: ESLint and Encapsulation**

Your template enforces the `use-encapsulation` rule, which requires state management to be in custom hooks. You'll see warnings throughout this lesson's exercises—that's expected.

**Why are we "breaking" the rule?**

- This lesson focuses on understanding **controlled inputs** (the `value`/`onChange` pattern)
- Adding hook encapsulation now would obscure what we're trying to learn
- Just like in the Immutability lesson, we practice with raw state first

**When will we fix it?**

In your final assessment, you'll create a `useRegistrationForm` custom hook that encapsulates all the form state and validation logic. The component will use the hook cleanly with no direct `useState` calls. This is the production pattern.

For now: **Ignore the ESLint warnings.** Focus on understanding controlled inputs.
