...
<Accordion question="Do you recall your HTML form validation attributes from your HTML course? 🤨">
...
- Use `src/form-validation-practice.js` for all Try It / Hands-On exercises.
...
Don't forget to ~~drink your Ovaltine~~ make regular commits with clear messages as you progress.
...
Create `src/form-validation-practice.js`. For this first step, the JS file can stay empty; we are focused on the HTML validation attributes (minlength, maxlength, type).
...
```html
<script type="module" src="./src/form-validation-practice.js"></script>
```
...
**Step 1:** Open `src/form-validation-practice.js`. Keep the form HTML in `index.html` exactly as you had it in Try It 1.

**Step 2:** In `src/form-validation-practice.js`, add this validation logic:
...
**Step 1:** Continue working in `src/form-validation-practice.js`.

**Step 2:** In the form HTML in `index.html`, add `pattern="[A-Za-z0-9]+"` to the username input.

**Step 3:** Below your existing JavaScript code in `form-validation-practice.js`, add this custom message handler:
...
**Step 1:** Still in `src/form-validation-practice.js`, below your existing code, add this counter logic:
...
**Step 1:** Finally, still in `src/form-validation-practice.js`, add (or modify) the submit handler to include error re-checking:
...
4. **Used vs remaining:** In practice, you counted `used / max` characters. In homework, you count `remaining`. Show the line of code you used for remaining characters, and explain the difference in words.
...
- [ ] Practice work completed in `src/form-validation-practice.js`
