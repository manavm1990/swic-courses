...
## Component Thinking: From Pages to Pieces

Modern web applications are built from **components** instead of one giant HTML file. Rather than thinking about a single static page, we identify meaningful parts of the UI (headers, buttons, cards, lists) and encapsulate each one into a **function component**.

These component functions:

- Return reusable UI (HTML + Tailwind classes) for a specific part of the page
- Can receive **parameters/props** so the same component renders different data
- Can be composed together to build larger interfaces

Because components are just functions, we can reuse them across different pages—or even in totally different apps—to create experiences that **look, feel, and behave consistently**.
