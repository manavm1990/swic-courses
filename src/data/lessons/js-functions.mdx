...
1. Read [Functions](https://eloquentjavascript.net/03_functions.html). Start with _Declaration Notation_ and stop at _Closures._
...
- `pokemonList` and `type` are **parameters** – placeholders defined in the function.
- When you call `filterByType(myTeam, "fire")`, the values `myTeam` and `"fire"` are the **arguments**.
...
1. **Ends** the function immediately – code after `return` in that function won't run.
