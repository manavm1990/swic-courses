...
3. **"Accidentally" delete content** - edit the file to just say:
...
6. **Verify it's back**:
   ...
   ```bash
   cat important.txt    # Mac/Linux
   type important.txt   # Windows
   ```

   Here, we are using another terminal command to display the contents of the file.
