import { CodeBlock, CopyableCode } from "@/components/code-block";
...
Just create `utils.test.js` in the same folder as your `utils.js` file. Add this boilerplate:
...
```javascript
import { getStudentPercentage } from "./utils.js";
import { test, expect } from "vitest";

test("calculates Maria's percentage correctly", () => {
  // Arrange: Create minimal test data for Maria only
  const data = {
    students: [
      {
        id: 1,
  ...
  const id = 1; // Maria's ID

  // Act: Call the function being tested
  const percentage = getStudentPercentage(data, id);

  // Assert: Verify the result
  expect(percentage).toBe(86); // * It's rounded...
});
```
