...
In particular, `dbaeumer.vscode-eslint` integrates ESLint into VS Code so that linting problems show up directly in your editor, and `usernamehw.errorlens` highlights those ESLint errors and warnings inline. That's why, when you violate a rule (like using `var` or leaving an unused variable), you see the issue immediately under the offending line of code.
...
1. It installed the necessary ESLint packages into `node_modules`. The packages were installed from the npm 📦 registry. This is what makes the `package.json` file critical. Without the list of the necessary packages, whenever we shared this project, others would not be able to install the required dependencies easily.
1. Last, but not least, it created an `eslint.config.js` file, which is the configuration file for ESLint. This file contains the rules and settings that ESLint will use to analyze your code. In the video ☝️ I trim this file down a bit, but it's not wholly necessary.

Later, when someone else clones this project and runs `npm install`, npm uses `package.json` (and `package-lock.json`) to reinstall all of those dependencies into a fresh `node_modules/` folder on their machine.

Now, if we try this out by opening up `src/index.js` and typing some 💩 code like: `var x = 5`. ESLint will start to warn us that we have declared `x` but haven't used it anywhere in the code. Now, this is a bit silly b/c if we just created the variable, of course we haven't used it yet.
...
1. **`prefer-const: "error"`** - If a variable is never reassigned, it should be `const`
1. **`max-params: 3`** - Functions with many parameters are hard to understand
1. **`eqeqeq: "always"`** - Use `===` instead of `==` to avoid type coercion bugs
1. **`no-console`** - Use specific methods like `console.info()` instead of `console.log()`
...
For the initial commit, this is not written with an imperative verb. It's 🆗 for this initial commit.
...
gh auth status
github.com
  ✓ Logged in to github.com account manavm1990 (keyring)
```
...
? Description
? Visibility Public
✓ Created repository manavm1990/node-template-repo on github.com
  https://github.com/manavm1990/node-template-repo
? Add a remote? Yes
? What should the new remote be called? origin
✓ Added remote git@github.com:manavm1990/node-template-repo.git
...
✓ Pushed commits to git@github.com:manavm1990/node-template-repo.git
```
